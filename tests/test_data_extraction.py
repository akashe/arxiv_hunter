...
def test_get_results(arxiv_parser):
    ...
    assert list(df1.columns) == ["title", "published_date", "pdf_link", "summary", "pdf_text"]
...
def test_store_data(arxiv_parser):
    ...
    assert list(arxiv_parser.extracted_data.columns) == ["title", "published_date", "pdf_link", "summary", "pdf_text", "summary_length", "pdf_text_length"]
    
    # Check that the dataframe contains only entries from the last 60 days
    assert arxiv_parser.extracted_data["published_date"].max() <= datetime.now()
    
    assert arxiv_parser.extracted_data["published_date"].min() >= datetime.now() - timedelta(days=60)
    ...
    # Check that the dataframe contains only entries from the last 30 days
    assert arxiv_parser.extracted_data["published_date"].max() <= datetime.now()
    assert arxiv_parser.extracted_data["published_date"].min() >= datetime.now() - timedelta(days=30)
